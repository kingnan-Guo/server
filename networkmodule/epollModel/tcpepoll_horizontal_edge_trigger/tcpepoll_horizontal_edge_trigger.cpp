/*
 * 程序名：tcpepoll2.cpp，此程序用于演示采用epoll模型的 边缘触发。
 * 作者：吴从周
...
// 初始化服务端的监听端口。
int _initserver(int port);

int MY_TCP_EPOLL_HET_MAIN(int argc,char *argv[])
{
    if (argc != 2) { printf("usage: ./tcpepoll2 port\n"); return -1; }

    // 初始化服务端用于监听的socket。
    int listensock = _initserver(atoi(argv[1]));
    printf("listensock=%d\n",listensock);
        ...
        for (int ii=0;ii<infds;ii++)       // 遍历epoll返回的数组evs。
        {
            ...
            if (evs[ii].data.fd==listensock)
            {
                while (true)
                {
                    ...
                    ev.events = EPOLLOUT | EPOLLET;        // ET-边缘触发。 写事件
                    epoll_ctl(epollfd,EPOLL_CTL_ADD,clientsock,&ev);
                }
            }
            else
...
int TCP_EPOLL_HET_MAIN(int argc, char *argv[]){
    MY_TCP_EPOLL_HET_MAIN(argc, argv);
}




// 非阻塞的 连接返回都是  EAGAIN ， 如何判断是否真正链接，使用写入， 如果是 写人成功那么 连接成功
