...
// 初始化服务端的监听端口。
int tcpeppol_initserver(int port);

int MY_TCP_EPOLL_MAIN(int argc,char *argv[])
{
    if (argc != 2) { printf("usage: ./tcpepoll port\n"); return -1; }

    // 初始化服务端用于监听的socket。
    int listensock = tcpeppol_initserver(atoi(argv[1]));
    printf("listensock=%d\n", listensock);
    ...
    while (true)        // 事件循环。
    {
         ...
         * 参数：
         *   epfd：epoll_create()返回的epoll句柄。
         *   events：存放epoll_wait()返回的事件。
         *   maxevents：events数组的大小。
         *   timeout：等待超时时间，单位是毫秒。
         *        -1：表示无限期等待。
         *   0：表示立即返回，不等待。
         * 返回值：
         *   成功：返回有多少个socket有事件发生。
         *   失败：返回-1，并设置errno。
         * 
         * 
         * 
        ...
        for (int ii=0;ii<infds;ii++)       // 遍历epoll返回的数组evs。
        {
            ...
            if (evs[ii].data.fd == listensock)
            {
                ...
                ev.events = EPOLLIN; // epoll 的 读事件
                epoll_ctl(epollfd, EPOLL_CTL_ADD, clientsock, &ev); // 把需要监视的socket和事件加入 epollfd 句柄中。
            }
            else
            {
...
int TCP_EPOLL_MAIN(int argc, char *argv[]){
    MY_TCP_EPOLL_MAIN(argc, argv);
}